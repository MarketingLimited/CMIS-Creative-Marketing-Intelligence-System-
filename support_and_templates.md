...
- [MEDIA_TEMPLATES — قوالب الوسائط](#media-templates-hub)
  - [قوالب الفيديو الجاهزة](#قوالب-الفيديو-الجاهزة)
  - [قوالب الصوت الجاهزة](#قوالب-الصوت-الجاهزة)
- [QUALITY_AND_VARIATIONS — الامتثال والتنويعات](#quality-and-variations)
- [OPERATIONS_AND_SUPPORT — التشغيل والدعم](#operations-and-support)
  - [تكامل Gems & GPTs](#gems--gpts-تكامل)
- [USER_AND_TRAINING_GUIDES — أدلة الاستخدام والتدريب](#user-and-training-guides)
<!-- CMIS:END::SUPPORT_AND_TEMPLATES_TOC -->

<!-- CMIS:START::MEDIA_TEMPLATES -->
<a id="media-templates-hub"></a>
## MEDIA_TEMPLATES — قوالب الوسائط
<!-- CMIS:START::MEDIA_VIDEO -->
<a id="قوالب-الفيديو-الجاهزة"></a>
...
<a id="quality-and-variations"></a>
## QUALITY_AND_VARIATIONS — الامتثال والتنويعات
...
- **تنويع الوسائط إلزامي:** في حزمة Stage B الأساسية، وزّع الأصول بحيث تضمين صورة ثابتة (أو كاروسيل)، وفيديو قصير (Reel/TikTok)، وStory/صيغة عمودية تفاعلية عبر التنويعات الثلاثة. دوّن نوع الوسيط بوضوح داخل الميتاداتا (`media_type`) وضمن وصف التصميم.
...
<a id="operations-and-support"></a>
## OPERATIONS_AND_SUPPORT — التشغيل والدعم
...
<a id="gems--gpts-تكامل"></a>
### تكامل Gems & GPTs
- **Goals مسبقة الإعداد:** اضبط الـ GPT/Gem على وضعيات `full_strategy`, `quick_draft`, أو `compliance_only` بحسب المهمة، وتأكد من تمرير الهدف التسويقي ضمن `{awareness, conversion, retention}` ليتكامل مع قواعد Stage B.
- **تحميل الحزم:** فعّل هذا الملف مع `instruction_prompt.md` وبقية الوحدات في نفس الجلسة، ثم استخدم خيار **Replace** عند التحديث لضمان أن Gems وGPTs تعتمد آخر نسخة.
- **استدعاء القوالب:** استخدم محددات الأقسام (مثل `§MEDIA_TEMPLATES`، `§QUALITY_AND_VARIATIONS`) داخل مطالباتك ليصل النظام إلى القوالب/القوائم الملائمة فورًا.
- **مراقبة الامتثال:** اطلب من الـ GPT/Gem إرجاع حقل `compliance_report` استنادًا إلى مخطط JSON الوارد هنا، وفعّل آلية التصحيح التلقائي إذا ظهر تنبيه.
- **تخصيص السوق:** عرّف المتغيّرات الأساسية (الجمهور، القطاع، اللهجة) في بداية الطلب ليتمكن النظام من تفعيل نصائح الخبراء التسويقية واللهجات البحرينية تلقائيًا.
...
<a id="user-and-training-guides"></a>
## USER_AND_TRAINING_GUIDES — أدلة الاستخدام والتدريب
