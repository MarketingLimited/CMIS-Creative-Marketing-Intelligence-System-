...
- `emotional_triggers`: [FOMO, الطموح, الأمان, الانتماء, الفضول, القيمة/السعر]
- `segments`: [أمهات عاملات, طلاب جامعات, شباب 18–24, عائلات, SMBs B2B]
- `pains`: [ضيق الوقت, تعقيد الشراء, قلة الثقة, السعر المرتفع, جودة غير ثابتة]
- `marketing_frameworks`: [AIDA, PAS, 4U, FAB, PASTOR, StoryBrand]
- `marketing_strategies`: [UGC, Social proof, Offer-led, Educational, Interactive/Shoppable]
- `tones`: [ودية, واثقة, مهنية, مشوقة, فاخرة]
- `features`: مثال (ضع 10–15 حسب القطاع)
- `benefits`: مثال (ضع 6–10)
- `transformational_benefits`: [طمأنينة دائمة, صورة اجتماعية أفضل, روتين أسهل]
- `usps`: [توصيل 25 دقيقة, ضمان رضا, تصنيع محلي, جودة موثقة]
- `message_map`: [رسالة حسب شريحة/قناة — قصيرة]
- `proofs`: [تقييم 4.8/5, 500+ عميل, شهادة جهة X, دراسة حالة مختصرة]
...
- أدرج دائمًا **وصف تصميمي**:
  - `composition` (التكوين/المشهد)، `background`, `lighting`, `colors` (لوحة/درجات),  
    `focus/highlight`, `de_emphasize`, `element_positions` (مواضع النص/اللوغو/السعر),  
    `ratio` (1:1, 4:5, 9:16), `motion` (للفيديو), `text_overlay ≤ 8 كلمات`.
- **الصوت/الموسيقى** (إن وجِدت): نوع الصوت، اللهجة (يفضّل بحريني/خليجي عند الملاءمة)،  
  موسيقى مرخصة/خالية من الحقوق، وذكر `#إعلان` صوتيًا خلال 3 ثوانٍ.
